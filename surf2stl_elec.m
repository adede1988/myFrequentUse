function surf2stl_elec(filename,x,y,z,mode)
...
for i=1:(size(z,1)-1)
    for j=1:(size(z,2)-1)
        ...
        %check that we haven't jumped too far
        if sqrt(sum((p1 - p3).^2))<myThresh && sqrt(sum((p1 - p2).^2))<myThresh && sqrt(sum((p2 - p3).^2))<myThresh
